...
def log_security_event(db: Session, user_id: Optional[int], event_type: str, details: str, ip_address: str, user_agent: str = None):
    """Log security events"""
...
async def send_otp_email(email: str, otp_code: str, purpose: str):
    """Send OTP email with Jinja2 template"""
    try:
        subject = f"Your OTP Code for {purpose.replace('_', ' ').title()}"
...
def verify_otp(otp_verify: OTPVerify, request: Request, db: Session = Depends(get_db)):
    """Verify OTP code"""
    utc = pytz.utc
    otp = db.query(OTP).filter(
        and_(
            ...
            OTP.used.is_(False),
            OTP.expires_at > utc.localize(datetime.utcnow())
        )
    ).first()
    ...
    otp.used = True
    db.commit()
    log_activity(db, None, "OTP_VERIFIED", f"OTP for {otp_verify.email} ({otp_verify.purpose}) verified successfully", request.client.host)
...
def change_password(
    ...
    db.commit()
    log_activity(db, current_user.id, "PASSWORD_CHANGE", "User changed password", request.client.host)
...
def change_pin(
    ...
    db.commit()
    log_activity(db, current_user.id, "PIN_CHANGE", "User changed PIN", request.client.host)
...
def verify_pin(
    ...
    if not pwd_context.verify(pin_data.pin, current_user.pin_hash):
        log_security_event(db, current_user.id, "PIN_VERIFICATION_FAILED", "Invalid PIN entered", request.client.host)
        raise HTTPException(status_code=400, detail="Invalid PIN")
    
    log_activity(db, current_user.id, "PIN_VERIFIED", "PIN verified successfully", request.client.host)
    return {"message": "PIN verified successfully"}

@app.post("/api/register")
def signup(user_data: UserCreate, db: Session = Depends(get_db)):
    """User registration with fraud protection checks"""
    ...
    user_id = ''.join(secrets.choice(string.digits) for _ in range(10))
...
def login(user_data: UserLogin, db: Session = Depends(get_db)):
    """User login with fraud protection checks"""
    ...
    if user_data.device_fingerprint:
        existing_device = db.query(UserDevice).filter(
            ...
            UserDevice.device_fingerprint == user_data.device_fingerprint
        ).first()
...
async def send_email_notification(
    ...
    """Enhanced email notification with Jinja2 template support"""
    
    try:
        if template_type == "account_creation":
            template = env.get_template('account_creation.html')
        elif template_type == "deposit_confirmation":
            template = env.get_template('deposit_confirmation.html')
        elif template_type == "transfer_receipt":
            template = env.get_template('transfer_receipt.html')
        elif template_type == "login_alert":
            template = env.get_template('login_alert.html')
        elif template_type == "agent_approval":
            template = env.get_template('agent_approval.html')
        else:
            # Fallback to OTP template for other types
            template = env.get_template('otp_email.html')
        ...
        email_log = EmailNotification(
            user_id=context.get('user_id', 1),  # Default to admin user if not provided
            email=email,
            subject=subject,
