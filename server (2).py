...
async def send_otp_email(email: str, otp_code: str, purpose: str):
    ...
    try:
        ...
        html_content = template.render(
            subject=subject,
            title="Your OTP Code",
            message=f"You requested an OTP code for {purpose.replace('_', ' ').title()}.",
            otp_code=otp_code,
            expiry_minutes=10
        )
...
async def send_email_notification(
    ...
    try:
        # Use Jinja2 templates based on template_type
        if template_type == "deposit_evidence":
            template = env.get_template('deposit_evidence.html')
        elif template_type == "deposit_submission":
            template = env.get_template('deposit_submission.html')
        elif template_type == "deposit_confirmed":
            template = env.get_template('deposit_confirmed.html')
        else:
